...
class Paradox:
    ...
    def dumpMessage(self, indata):

        if indata is None or len(indata) != 37:
            return
        ...
        if data[0] == 0x72:
            m={
                    ...
                    "New Protocol": data[1],
                    "Protocol ID": data[2],
                    "Protocol Version": "%d.%d build %d" % (data[3], data[4], data[5]),
                    "Family:": data[6],
                    "Product:": data[7],
                    "Talker": ["Boot loader", "Controller application", "Module application, Unknown"][min(3, data[8])],
                    "Application Version": "%d.%d build %d" % (data[9], data[10], data[11]),
                    "Serial Number": "%02x%02x%02x%02x" % (data[12], data[13], data[14], data[15]),
                    "Hardware version": "%d.%d" % (data[16], data[17]),
                    "Bootloader version": "%d.%d build %d" % (data[18], data[19], data[20]),
                    "Bootloader Date": "%02d-%02d-%04d" % (data[21], data[22], data[23]*100+data[24]),
                    "Processor ID": data[25],
                    "Checksum ID": data[26],
                    "Encryption ID": data[27],
                    }
        elif hn == 0x01:
            m = {"Command": hn, 
                    "Message Center": ["NEware software is connected", "Winload software is connected", "Alarm reporting pending", "Future Use"][data[1]],
                    "Bytes in Packet": data[2],
                    "Type": ["Winload answer", "NEware answer"][(data[3]>>4) & 1],
                    "Callback": ["Enabled", "Disabled"][min(1,data[4])]
                }
        else:
            return
        
        logging.debug(json.dumps(m, indent=4))
